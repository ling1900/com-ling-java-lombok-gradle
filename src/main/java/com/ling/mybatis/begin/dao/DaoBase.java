...
public abstract class DaoBase<E extends EntityBase,Q extends QueryBase,U extends UpdateRequestBase> {
    ...
    protected void beforeInsert(E entityBase) {
        if (entityBase.getCreateTime() == null) {
            entityBase.setCreateTime(new Date());
        }
        if (entityBase.getLastUpdateTime() == null) {
            entityBase.setLastUpdateTime(entityBase.getCreateTime());
        }
        if (entityBase.getLastUpdateBy() == null) {
            entityBase.setLastUpdateBy(entityBase.getCreateBy());
        }
    }
